...
export default function CheckoutPage() {
  const { items, totalPrice, refreshPrices } = useCart();
  ...
  const placeOrder = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await response.json() as CreateOrderResponse | { error?: string };

      if (!response.ok || !("orderNumber" in result)) {
        if (response.status === 409) await refreshPrices();
        throw new Error("error" in result ? result.error : undefined);
      }

      window.location.assign(createWhatsAppOrder(details, result.items, result.subtotal, result.deliveryCharge, result.grandTotal, paymentMethod, result.orderNumber));
    } catch (error) {
      setIsPlacingOrder(false);
      setError(error instanceof Error ? error.message : "We couldn't place your order. Please try again.");
    }
  };

  if (items.length === 0) {
    return <><Header /><main className="bg-[var(--cream)] px-4 py-16 sm:py-24"><div className="mx-auto max-w-xl rounded-3xl border border-[var(--line)] bg-white px-6 py-16 text-center shadow-sm"><p className="text-lg font-bold text-[var(--navy)]">Your cart is empty.</p><p className="mt-2 text-sm text-slate-500">Add something fresh before checking out.</p><Link href="/products" className="mt-7 inline-flex rounded-full bg-[var(--navy)] px-6 py-3 text-sm font-bold text-white">Browse Products</Link></div></main><Footer /></>;
  }

  return (
    <><Header /><main className="bg-[var(--cream)] pb-16 pt-10 sm:pb-24 sm:pt-14">
